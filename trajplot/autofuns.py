...
class Repr( Figure ) :
	...
	def df( self , d ) : # increment frames by d
		...
		old_frame = self._f		# store the old frame, in case there are
								# only nan until the end of the trajectory
								# file
		while c[ 0 ] != c[ 0 ] :
					
			if ( ( self._f + d >= self._fmin ) & ( self._f + d <= self._fmax ) ) :
...
def icheck( tt , path_movies = '' , path_datasets = '' , path_movie = '' , r = 7 , cmap = 'gray' , path_output = './' , marker = 's' , markersize = 25 , offset = ( 0 , 0 ) , anticipate = 0 ) :
	...
	def ZoomIn( event , frame , increment = -1 ) :
...
#			GUI_plot( tt , 1 )
	...
	frm = Repr( tt , 0 , r , LoadMovie( path_movies , path_movie , tt[ 0 ] ) )
...
#	SpotWindow.bind( "-" , lambda event , frame = frm : ZoomOut( event , frame ) )
#	SpotWindow.bind( "+" , lambda event , frame = frm : ZoomIn( event , frame ) )
#	
	SpotWindow.mainloop()
